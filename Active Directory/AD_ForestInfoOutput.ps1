<# 
.SYNOPSIS
This script gathers comprehensive information about an Active Directory forest, including forest-wide and domain-specific data such as AD Recycle Bin status, tombstone lifetime, schema version, domain controllers, trusts, operating systems, groups, users, and more.

.DESCRIPTION
This PowerShell script retrieves and compiles extensive information about an Active Directory forest, covering various aspects like forest functional level, domain controllers, trusts, group policies, users, groups, operating systems, and more. The collected data is presented in an HTML report format for easy viewing and analysis.

.NOTES
Script Name: ForestInfo.ps1
Version: 1.0
...
$frag4 = $AllSubnet | Select-Object Site,subnet |ConvertTo-Html -property site,subnet -head $a -PreContent '<h2>Subnets information</h2>' |Out-String
#---------------------------------------------------------------------------------------------------------------------------------------------------
[Array]$siteLinks = Get-ADObject -Server $forest -Filter { objectClass -eq "siteLink" } -SearchBase $ConfigurationPart -Properties name, cost, replInterval, siteList | Sort-Object replInterval
...
ConvertTo-HTML -head $a -PostContent $frag1,$frag3,$frag4,$frag5,$frag11,$fra9,$timing -PreContent $precontenteND | Out-File C:\Temp\AD_ForestInfo.html