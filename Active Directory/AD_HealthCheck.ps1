...
# Date: 2023-07-20
# Version: 1.0
# Description: This script performs a comprehensive health check of Active Directory (AD) services and components across domain controllers. It checks ping connectivity, status of key services (Netlogon, NTDS, DNS), and runs AD diagnostic tests (Netlogons, Replications, Services, Advertising, FSMOCheck). The script generates an HTML report with detailed results, indicating the health status of each tested component.
#############################################################################
###########################Define Variables##################################
...
$smtphost = "relay.repsrv.com" 
$from = "prodpsapp02@repsrv.com" 
$email1 = "vsanchez2@republicservices.com, LGonzalez@republicservices.com, Christen.Gay@republicservices.com"
$timeout = "60"
...
Write-Host $DC `t $DC `t Ping Success -ForegroundColor Green
		...
		$serviceStatus = start-job -scriptblock {get-service -ComputerName $($args[0]) -Name "DNS" -ErrorAction SilentlyContinue} -ArgumentList $DC
            ...
            if($serviceStatus.state -like "Running")
                ...
                {
                $serviceStatus1 = Receive-job $serviceStatus
                if ($null -eq $serviceStatus1.status) {
...
if(!($body -like "*fail*" -or $body -like "*stop*")){ Write-Host "all tests passed"
$today =Get-date
$Day=$today.DayOfWeek
$hour = $today.Hour
if($day -eq 'Wednesday' -and $hour -eq 7){Write-Host "All tests passed, send a message"
$subject = "Healthy Active Directory " 
#$body = Get-Content "c:\tenp\ADreport.htm" 
...
if($body -like "*fail*" -or $body -like "*stop*"){ Write-Host " Failure!!!!!!!"
$email1 = "vsanchez2@republicservices.com, LGonzalez@republicservices.com, Christen.Gay@republicservices.com,itsupport@repsrv.com"
$subject = "Failure!!!!! Active Directory Health Monitor" 
#$body = Get-Content "c:\temp\ADreport.htm" 
$smtp= New-Object System.Net.Mail.SmtpClient $smtphost 
$msg = New-Object System.Net.Mail.MailMessage 
$msg.To.Add($email1)
...
########################################################################################
        ...
        # if(!($body -like "*fail*" -or $body -like "*stop*")){ Write-Host " all tests passed"}
        # Else {Write-Host "Health check failed"}